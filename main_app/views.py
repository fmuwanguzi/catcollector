...
def login_view(request):
    #if post, then authenitcate. User will be submitting username and password
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                print('the usernamen and/or password is incorrect.')
        else:
            form = AuthenticationForm()
            return render(request, 'login.html', {'form': form})
    else:# get request that sent up an empty form
        form = AuthenticationForm()
        return render(request, 'login.html', {'form': form })
